"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var _a;
Object.defineProperty(exports, "__esModule", { value: true });
exports.WikiController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const wiki_service_1 = require("./wiki.service");
let WikiController = class WikiController {
    constructor(wikiService) {
        this.wikiService = wikiService;
    }
    findAll() {
        return this.wikiService.findAll();
    }
    findOne(id) {
        return this.wikiService.findOne(id);
    }
    create(createWikiDto) {
        return this.wikiService.create(createWikiDto);
    }
    getTemplates() {
        return this.wikiService.getElektrotechnikTemplates();
    }
};
exports.WikiController = WikiController;
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Alle Wiki-Artikel abrufen' }),
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], WikiController.prototype, "findAll", null);
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Wiki-Artikel nach ID abrufen' }),
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], WikiController.prototype, "findOne", null);
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Neuen Wiki-Artikel erstellen' }),
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], WikiController.prototype, "create", null);
__decorate([
    (0, swagger_1.ApiOperation)({ summary: 'Elektrotechnik-Templates abrufen' }),
    (0, common_1.Get)('templates/all'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], WikiController.prototype, "getTemplates", null);
exports.WikiController = WikiController = __decorate([
    (0, swagger_1.ApiTags)('wiki'),
    (0, common_1.Controller)('wiki'),
    __metadata("design:paramtypes", [typeof (_a = typeof wiki_service_1.WikiService !== "undefined" && wiki_service_1.WikiService) === "function" ? _a : Object])
], WikiController);
//# sourceMappingURL=wiki.controller.js.map